...
export class AfipClass {
  ...
  async getDataCUIT(cuit_cons: number): Promise<{
    ...
    const dataCUIT = await this.afip.RegisterScopeFive.getTaxpayerDetails(
      cuit_cons,
    );
    if (dataCUIT === null) {
      const response = {
  ...
  async lastFact(
    ...
    try {
      const ultFact = await this.afip.ElectronicBilling.getLastVoucher(
        pv,
        tipo,
      );
      const response = {
  ...
  async getInvoiceInfo(
    ...
    try {
      const dataFact = await this.afip.ElectronicBilling.getVoucherInfo(
        ncbte,
        pv,
        tipo,
      );
...
export interface FactMonotribProd
  extends FactBase,
    FactMonotrib,
    FactProductos {}
export interface FactMonotribServ
  extends FactBase,
    FactMonotrib,
    FactServicios {}

export interface FactInscriptoProd
  extends FactBase,
    FactInscriptos,
    FactProductos {}
export interface FactInscriptoServ
  extends FactBase,
    FactInscriptos,
    FactServicios {}
