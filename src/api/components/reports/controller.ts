    ...
    const products = async (fromDate: string, toDate: string, pvId?: number, idProd?: number) => {
        ...
        if (idProd) {
            filter = {
                ...
                concat: EConcatWhere.none,
                items: [
                    { column: `${Tables.DET_FACTURAS}.${Columns.detallesFact.id_prod}`, object: String(idProd) },
                ]
            };
            filters.push(filter);
        }
        ...
        filter = {
            ...
            concat: EConcatWhere.none,
            items: [
                { column: `${Tables.FACTURAS}.${Columns.facturas.fecha}`, object: String(fromDate) },
            ]
        };
        filters.push(filter);
        ...
        filter = {
            ...
            concat: EConcatWhere.none,
            items: [
                { column: `${Tables.FACTURAS}.${Columns.facturas.fecha}`, object: String(toDate) },
            ]
        };
        filters.push(filter);
        ...
        const data = await store.list(Tables.FACTURAS, [
